...
/**
 * Test cases for {@link PlayerVote}.
 *
 * @author Rick van der Heijden (1461923)
 * @since 21-02-2022
 */
public class PlayerVoteTest extends VoteTest {

    @Override
    protected Vote createNewInstance() {
        try {
            return new PlayerVote(super.gameInstanceID, VoteType.MISC, new ArrayList<>());
    ...
    public void constructorSingleParameterTest1() {
        try {
            ...
            int vid = (new PlayerVote(super.gameInstanceID, type, new ArrayList<>())).getVid();

            // Test the single parameter constructor
            Vote vote = new PlayerVote(vid);
    ...
    public void constructorSingleParameterTest2() {
        try {
            ...
            // Create the vote
            int vid = (new PlayerVote(super.gameInstanceID, type, new ArrayList<>(allowed))).getVid();
    ...
    public void constructorMultiParameterTest1() {
        try {
            ...
            Vote vote = new PlayerVote(super.gameInstanceID, type, new ArrayList<>());
    ...
    public void constructorMultiParameterTest2() {
        try {
            ...
            allowed.add(henk.getPlayer());
            allowed.add(jan.getPlayer());
            Vote vote = new PlayerVote(super.gameInstanceID, type, new ArrayList<>(allowed));
            List<PlayerIdentifier> allowedIDs = allowed.stream().map(Player::getPlayerIdentifier).toList();
    ...
    public void getAllowedTest1() {
        try {
            ...
            allowed.add(henk.getPlayer());
            allowed.add(jan.getPlayer());
            PlayerVote vote = new PlayerVote(super.gameInstanceID, VoteType.MISC, new ArrayList<>(allowed));
            List<PlayerIdentifier> allowedIDs = allowed.stream().map(Player::getPlayerIdentifier).toList();
     ...
     * no {@code Player}s are eligible.
     * @utp.test_items {@code PlayerVote}
     * @utp.input_specs -
    ...
    public void startEndBasicTest1() {
        try {
            PlayerVote vote = new PlayerVote(super.gameInstanceID, VoteType.MISC, new ArrayList<>());
            vote.start();
            assertNull(vote.end());
        } catch (SQLException e) {
            fail("Should not have thrown an SQL exception.");
     ...
     *
     * @utp.description Tests whether the result of a {@code Vote} with zero {@code Ballot}s cast is {@code null} when
     * at least some {@code Player}s are eligible.
     * @utp.test_items {@code PlayerVote}
     * @utp.input_specs -
    ...
    public void startEndBasicTest2() {
        try {
            ...
            allowed.add(henk.getPlayer());
            allowed.add(jan.getPlayer());
            PlayerVote vote = new PlayerVote(super.gameInstanceID, VoteType.MISC, allowed);
            vote.start();
            assertNull(vote.end());
        } catch (SQLException | GameException e) {
            fail("Should not have thrown an exception.");
     ...
     *
     * @utp.description Tests whether voting is impossible before a {@code PlayerVote} is opened, and whether voting is
     * possible after a {@code PlayerVote} has been opened.
     * @utp.test_items {@code PlayerVote}
     * @utp.input_specs -
     * @utp.output_specs Whether voting throws a {@code VoteNotStartedException} before a {@code PlayerVote} is opened,
     * and whether voting succeeds after a {@code PlayerVote} has been opened.
     * @utp.env_needs -
     */
    @Test
     ...
     *
     * @utp.description Tests whether the result of a {@code PlayerVote} with 2 {@code Player}s eligible for receiving
     * votes is correct.
     * @utp.test_items {@code PlayerVote}
     * @utp.input_specs -
     * @utp.output_specs Whether the result of a {@code PlayerVote} in which {@code Player} Henk votes for {@code
